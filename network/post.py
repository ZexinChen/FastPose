...
def NMS(param, heatmaps,upsampFactor=1.,NUM_JOINTS_in=18,  bool_refine_center=True, bool_gaussian_filt=False):
    """
    ...
    for joint in range(NUM_JOINTS_in):
...
def decode_pose_test(img_orig, param, heatmaps, pafs):
    # Bottom-up approach:
    # Step 1: find all joints in the image (organized by joint type: [0]=nose,
    # [1]=neck...)
    ...
    if len(pafs)==0:
        joint_list_per_joint_type = NMS(param,
                                    heatmaps, img_orig.shape[0] / float(heatmaps.shape[0]),NUM_JOINTS_in=36)
        # joint_list is an unravel'd version of joint_list_per_joint, where we add
        # a 5th column to indicate the joint_type (0=nose, 1=neck...)
        joint_list = np.array([tuple(peak) + (joint_type,) for joint_type,
                               joint_peaks in enumerate(joint_list_per_joint_type) for peak in joint_peaks])
        ...
        joint_list = np.array([tuple(peak) + (joint_type,) for joint_type,
                               joint_peaks in enumerate(joint_list_per_joint_type) for peak in joint_peaks])
        middle_joint_list_per_joint_type = NMS(param,
                                        pafs, img_orig.shape[0] / float(pafs.shape[0]))
        # joint_list is an unravel'd version of joint_list_per_joint, where we add
        # a 5th column to indicate the joint_type (0=nose, 1=neck...)
        middle_joint_list = np.array([tuple(peak) + (joint_type,) for joint_type,
                               joint_peaks in enumerate(middle_joint_list_per_joint_type) for peak in joint_peaks])
    ...
    return joint_list, middle_joint_list

def outputChangeForm(joint_list, middle_joint_list):
    joint_list_list = [[] for i in range(18)]
    middle_joint_list_list = [[] for i in range(18)]

    for joint in joint_list:
        joint_list_list[int(joint[4])].append(joint)
    for middle_joint in middle_joint_list:
        # print(int(middle_joint[4]),middle_joint)
        middle_joint_list_list[int(middle_joint[4])].append(middle_joint)

    return joint_list_list,middle_joint_list_list

def outputChangeForm_test(joint_list):
    joint_list_list = [[] for i in range(18)]
    middle_joint_list_list = [[] for i in range(19)]

    for joint in joint_list:
        if int(joint[4])>17:
            middle_joint_list_list[int(joint[4])-18].append(joint)
        else:
            joint_list_list[int(joint[4])].append(joint)
            # if int(joint[4])==1:#neck
            #     middle_joint_list_list[4].append(joint)
