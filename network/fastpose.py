...
def get_model(trunk='vgg19'):
    ...
    blocks['block1_2'] = [{'conv5_1_CPM_L2': [128, 128, 3, 1, 1]},
                          ...
                          {'conv5_5_CPM_L2': [512, 37, 1, 1, 0]}]

    # Stages 2 - 6
    for i in range(2, 7):

        blocks['block%d_2' % i] = [
            {'Mconv1_stage%d_L2' % i: [165, 128, 7, 1, 3]},
            {'Mconv2_stage%d_L2' % i: [128, 128, 7, 1, 3]},
            {'Mconv3_stage%d_L2' % i: [128, 128, 7, 1, 3]},
            {'Mconv4_stage%d_L2' % i: [128, 128, 7, 1, 3]},
            {'Mconv5_stage%d_L2' % i: [128, 128, 7, 1, 3]},
            {'Mconv6_stage%d_L2' % i: [128, 128, 1, 1, 0]},
            {'Mconv7_stage%d_L2' % i: [128, 37, 1, 1, 0]}
        ]
...
def test():
    ...
    start = time.clock()
    for i in range(20):
        model(Variable(torch.randn(1,3,368,368).cuda()))
    end = time.clock()
    print('average forward time:%f'%(float(end-start)/20.0))
